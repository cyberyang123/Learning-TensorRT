...
int main(int argc, char **argv)
{
    ...
    std::string file_path = "yolov8x.engine";
    ...
    float cal_time_pre, cal_time_inf, cal_time_pos = 0.0;
    ...
    for(int i = 1; i < 63; i += 1)
    {
        ...
        cal_time_pre += ((double)(end-start)/CLOCKS_PER_SEC) * 1000;
        //推理
        start = clock();
        ...
        cal_time_inf += ((double)(end-start)/CLOCKS_PER_SEC) * 1000;
        //后处理
        start = clock();
        ...
        cal_time_pos += ((double)(end-start)/CLOCKS_PER_SEC) * 1000;
    }

    std::cout << "yolov8x : preprocess:" << cal_time_pre/62 << "ms; inference: " << cal_time_inf/62 << "ms; postprocess:" << cal_time_pos/62 << "ms; fps:" << 1000/(cal_time_pre+cal_time_inf+cal_time_pos)*62 << std::endl;
